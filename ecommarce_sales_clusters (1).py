...
x = rfm[["Recency", "Frequency", "Monetary"]]  # Adjust if needed
...
for cluster in rfm["Cluster"].unique():
    ...
    # Store the model and scaler for prediction use
    rf_models[cluster] = {
        "model": rf,
        "scaler": scaler
    }
...
def predict_cluster(recency, frequency, monetary):
    # Prepare one row for prediction
    new_customer = pd.DataFrame(
        [[recency, frequency, monetary]],
        columns=["Recency", "Frequency", "Monetary"]
    )

    # Scale using kmeans scaler
    new_scaled = scaler_kmeans.transform(new_customer)
